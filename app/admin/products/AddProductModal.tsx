...
import { PRODUCT_CATEGORIES } from './categories'; // ✨ Importation de notre dictionnaire
...
export default function AddProductModal({ isOpen, onClose }: AddProductModalProps) {
    ...
    // Etats pour gérer le filtrage dynamique des catégories
    ...
    return (
        <div className="fixed inset-0 bg-[#2c3e50]/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div className="bg-white rounded-[2rem] border border-gray-100 w-full max-w-lg p-8 shadow-2xl relative my-8 animate-in zoom-in-95 duration-150 text-left">
                ...
                <h3 className="text-xl font-bold text-[#2c3e50] mb-1">Nouveau Produit</h3>
                <p className="text-xs text-gray-400 font-medium mb-6">Ajoutez une nouvelle création à ton catalogue Ndanty avec catégorie et sous-catégorie.</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <input type="text" name="name" required placeholder="Ex: Table basse en teck" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-xs font-semibold focus:bg-white focus:ring-1 focus:ring-[#28a745] focus:border-[#28a745] outline-none transition-all" />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1">
                            ...
                            <input type="number" name="price" required min="0" placeholder="0" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-xs font-semibold focus:bg-white focus:ring-1 focus:ring-[#28a745] focus:border-[#28a745] outline-none transition-all" />
                        </div>
                        {/* STOCK */}
                        <div className="space-y-1">
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Quantité en stock</label>
                            <input type="number" name="stock" defaultValue="1" min="0" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-xs font-semibold focus:bg-white focus:ring-1 focus:ring-[#28a745] focus:border-[#28a745] outline-none transition-all" />
                        </div>
                    </div>
                    ...
                    <div className="space-y-1">
                        ...
                        <textarea name="description" rows={3} placeholder="Détails, essences de bois, finitions..." className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-xs font-semibold focus:bg-white focus:ring-1 focus:ring-[#28a745] focus:border-[#28a745] outline-none transition-all resize-none" />
                    </div>
